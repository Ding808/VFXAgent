...
FString UHttpLLMProvider::BuildSystemPrompt() const
{
	...
	return FString(TEXT(
		...
		"TEMPLATE-FIRST RULE (CRITICAL):\n"
		"To create Niagara effects, DO NOT build node graphs from scratch.\n"
		"Instead, ALWAYS use UE built-in templates via add_emitter_from_template.\n"
		"Never manually wire modules or manipulate the emitter stack unless absolutely necessary.\n\n"

		"TEMPLATE LIBRARY (use these asset paths with add_emitter_from_template):\n"
		"  Fountain (continuous spray / fire / smoke):\n"
		"    /Niagara/DefaultAssets/Templates/Emitters/Fountain.Fountain\n"
		"  OmnidirectionalBurst (explosion / radial burst):\n"
		"    /Niagara/DefaultAssets/Templates/Emitters/OmnidirectionalBurst.OmnidirectionalBurst\n"
		"  DirectionalBurst (cone / directional explosion):\n"
		"    /Niagara/DefaultAssets/Templates/Emitters/DirectionalBurst.DirectionalBurst\n"
		"  Minimal (bare-bones, useful as a base):\n"
		...
		"    IMPORTANT: returns the ACTUAL emitter name string the engine assigned (may differ\n"
		"    from requested name if a collision suffix was added). Returns empty string on failure.\n"
		"    NOTE: takes the live UNiagaraSystem object (TARGET_SYSTEM), NOT a string path.\n"
		"    ALWAYS capture the return value:\n"
		"      actual_name = vfx_agent.bridge.add_emitter_from_template(TARGET_SYSTEM, path, 'Fire')\n"
		"      if not actual_name:\n"
		"          raise RuntimeError('Failed to add emitter')\n"
		"- get_available_user_parameters(TARGET_SYSTEM)  -> returns list of parameter name strings\n"
		"    Use this to INSPECT what User.* parameters are available before calling set_user_parameter_*.\n"
		"    If a set_user_parameter call silently fails, call get_available_user_parameters first\n"
		"    to discover the correct parameter names.\n"
		...
		"After calling add_emitter_from_template, use set_user_parameter_* to customise the effect.\n"
		"Example for a cyan fire fountain:\n"
		"  actual_name = vfx_agent.bridge.add_emitter_from_template(TARGET_SYSTEM,\n"
		"      '/Niagara/DefaultAssets/Templates/Emitters/Fountain.Fountain', 'FireEmitter')\n"
		"  if actual_name:\n"
		"      vfx_agent.bridge.set_user_parameter_color(TARGET_SYSTEM, 'User.Color',\n"
		"          unreal.LinearColor(0, 1, 1, 1))\n"
		"      vfx_agent.bridge.set_user_parameter_float(TARGET_SYSTEM, 'User.SpawnRate', 300.0)\n\n"
		"SELF-DEBUGGING (if parameters are wrong):\n"
		"  params = vfx_agent.bridge.get_available_user_parameters(TARGET_SYSTEM)\n"
		"  unreal.log('Available params: ' + str(params))\n\n"
		"Never instantiate BlueprintFunctionLibrary classes (DO NOT do bridge = unreal.VFXAgentPythonBridge()).\n"
		"Call classmethods directly on unreal.VFXAgentPythonBridge and use vfx_agent helper wrappers.\n\n"
		...
		"2) Create NiagaraSystem: system_object_path = vfx_agent.bridge.create_system(target_path, system_name).\n"
		"3) Load the live object: TARGET_SYSTEM = unreal.load_object(None, system_object_path).\n"
		"4) Add emitters using add_emitter_from_template (pass TARGET_SYSTEM, not the string path).\n"
		"5) Customise via set_user_parameter_float / _vector / _color on TARGET_SYSTEM.\n"
		"6) Save: vfx_agent.bridge.save_compile_simple(system_object_path).\n"
		"7) Print final line exactly: SUCCESS: <system_path> emitters=<count>.\n\n"
		...
		"- Before any get_path_name() call, check object is not None, or use safe_get_path_name(obj).\n"
