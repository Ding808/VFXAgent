...
#if 0 // Old implementation disabled - kept for reference
...
bool UNiagaraSystemGenerator::UpdateNiagaraSystem(UNiagaraSystem* System, const FVFXRecipe& Recipe)
{
	...
	for (int32 Index = 0; Index < Recipe.Emitters.Num(); ++Index)
	{
		...
		if (TargetTemplate)
		{
			// Use DuplicateWithoutMerging to create a standalone copy that doesn't inherit from the parent template
			EmitterAsset = TargetTemplate->DuplicateWithoutMerging(System);
			if (EmitterAsset)
			{
				...
				EmitterAsset->Rename(*UniqueObjectName.ToString(), System, REN_DontCreateRedirectors);
				// Clear the parent reference to ensure the emitter is fully standalone
				EmitterAsset->SetParent(FVersionedNiagaraEmitter());
				// Disable versioning to completely sever the emitter from any template/version chain
				const FGuid NewVersionGuid = FGuid::NewGuid();
				EmitterAsset->DisableVersioning(NewVersionGuid);
			}
		}
		...
		// CRITICAL FIX: Use a new GUID to break the inheritance link from the template.
		// Using the original VersionGuid would cause Niagara to treat this as a template-inherited emitter (locked).
		const FGuid NewVersionGuid = FGuid::NewGuid();
		System->AddEmitterHandle(*EmitterAsset, FName(*EmitterNameStr), NewVersionGuid);
	}
...
#endif // Old implementation disabled